...
const WORKER_URLS = [
  "https://dainty-lolly-9036c6.netlify.app",
  "https://tangerine-bubblegum-a7047b.netlify.app",
  "https://lustrous-frangipane-1d4ce9.netlify.app",
  "https://hilarious-rugelach-6767a8.netlify.app",
];
...
export function WorkerTestPart() {
  const workerList = useMemo(() => {
    return WORKER_URLS.map((url, ind) => ({
      id: ind.toString(),
      url,
    }));
  }, []);
    ...
    <>
      ...
      <Box>
        {workerList.map((v, i) => {
          ...
          if (!s) return <WorkerItem name={name} url={v.url} key={v.id} />;
          if (s.status === "error")
            return (
              <WorkerItem
                name={name}
          ...
          return <WorkerItem name={name} url={v.url} key={v.id} />;
        })}
        <Divider />
